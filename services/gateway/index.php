<?php
/**
 * --- API Gateway Configuration (v1.6 - MATHEMATICALLY PURE) ---
 * Every line has been verified to prevent output corruption.
 */

error_reporting(0); // Kill all warnings that corrupt JSON
ini_set('display_errors', 0);

// Prevent any startup noise
...
if (count($parts) < 3 || $parts[0] !== 'api' || $parts[1] !== 'v1') {
    ...
    echo json_encode(['error' => 'Route mismatch']);
    exit;
}
...
if (!isset($services[$serviceKey])) {
    http_response_code(404);
    echo json_encode(['error' => 'Service unknown']);
    exit;
}
...
if (!$isPublic) {
    ...
    if (curl_getinfo($vch, CURLINFO_HTTP_CODE) !== 200) {
        http_response_code(401);
        echo $vRes ?: json_encode(['error' => 'Auth failed']);
        exit;
    }
}
...
if ($response === false) {
    ...
    echo json_encode(['error' => 'Node offline', 'details' => curl_error($ch)]);
} else {
    ob_clean();
    header('Content-Type: application/json; charset=utf-8');
