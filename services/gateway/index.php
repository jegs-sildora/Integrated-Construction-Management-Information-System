<?php
/**
 * --- API Gateway Configuration (v1.7 - PURE DATA PROXY) ---
 */

error_reporting(0);
ini_set('display_errors', 0);

// Nuke any previous buffers
...
if (count($parts) < 3 || $parts[0] !== 'api' || $parts[1] !== 'v1') {
    ...
    die(json_encode(['error' => 'Route mismatch']));
}

$serviceKey = $parts[2];
if (!isset($services[$serviceKey])) {
    http_response_code(404);
    die(json_encode(['error' => 'Service unknown']));
}
...
foreach (apache_request_headers() as $k => $v) {
    // CRITICAL: Filter headers that cause corruption or loops
    $lowK = strtolower($k);
...
if ($response === false) {
    ...
    // Send raw body from microservice
    ob_clean();
