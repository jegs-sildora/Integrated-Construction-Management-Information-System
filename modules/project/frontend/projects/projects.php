<?php
  ...
  require_once __DIR__ . '/../../../../config/database.php'; // Main DB
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <!-- Module-Specific CSS (Original project CSS files) -->
  <link rel="stylesheet" href="../components/css/style.css">
  <link rel="stylesheet" href="../components/css/tab.css">
  <link rel="stylesheet" href="../components/css/tablefooter.css">
  <link rel="stylesheet" href="../components/css/ccontrolbar.css">
  <link rel="stylesheet" href="../components/css/ctables.css">
  <link rel="stylesheet" href="../components/css/cmodal.css">
  <link rel="stylesheet" href="../components/css/ccards.css">
...
<body class="bg-gray-50">
  <?php 
    ...
    // 5. UI Components (Toast)
    include __DIR__ . '/../../../../includes/toast.php';
  ?>
  
  <?php
    // 6. Header Variables & Include
  ...
  <?php
    // 7. Fetch Projects from Database
    $sql = "SELECT p.*, 
    ...
    if ($result && $result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        ...
        if ($status === 'planning' || $status === 'in progress') $activeCount++;
        ...
        <table class="w-full" id="generalTable">
          ...
          <tbody class="divide-y divide-gray-200" id="projectsTableBody">
            ...
            <tr class="hover:bg-gray-50 transition-colors duration-150 project-row" 
                ...
                data-budget="<?php echo floatval($project['budget'] ?? 0); ?>"
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div class="text-xs text-gray-500">#<?php echo $project['project_id']; ?></div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4">
                ...
                <div class="text-xs text-gray-500">#<?php echo $project['project_manager_id']; ?></div>
                <?php endif; ?>
              </td>
              <td class="px-6 py-4">
                <div class="text-sm text-gray-600 max-w-xs truncate"><?php echo htmlspecialchars($project['description'] ?? '-'); ?></div>
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="text-sm font-semibold text-gray-900">₱<?php echo number_format($project['budget'] ?? 0, 2); ?></span>
              </td>
  ...
  <div id="deleteModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full transform transition-all animate-modal-slide-in">
      ...
      <div class="p-8">
        ...
        <p class="text-gray-500 text-sm mb-6">This action is permanent and cannot be undone.</p>
  ...
  <!-- JavaScript Libraries (Original project scripts) -->
  <script src="../components/js/jquery.min.js"></script>
  <script src="../components/js/animateStats.js"></script>
  <script src="../components/js/tableManager.js"></script>
  <script src="../components/js/showToast.js"></script>
  <script src="../components/js/filter.js"></script>
  ...
  <script>
    ...
    document.getElementById('addProjectBtn')?.addEventListener('click', function() {
      ...
      fetch(backendUrl + '?get_next_id=1')
        .then(res => res.json())
        .then(data => {
          if (data.success) document.getElementById('project_id').value = data.project_id;
        });

      // Fetch active employees for manager select
      fetch('../../backend/employee/backend_employee.php?fetch_all=1')
        .then(res => res.json())
        .then(data => {
          if (data.success) {
            ...
            data.employees.forEach(emp => {
              if (emp.status.toLowerCase() === 'active') {
                ...
                select.innerHTML += `<option value="${emp.employee_id}">${fullName} (${emp.employee_id})</option>`;
              }
            });
          }
        });
                  ...
                  if (empData.success) {
                    ...
                    empData.employees.forEach(emp => {
                      if (emp.status.toLowerCase() === 'active') {
                        ...
                        select.innerHTML += `<option value="${emp.employee_id}" ${selected}>${fullName}</option>`;
                      }
                    });
