...
document.getElementById('addProjectBtn')?.addEventListener('click', function() {
    ...
    fetch(`${window.GATEWAY_URL}project/projects/next-code`, {
        headers: { 'Authorization': `Bearer ${window.AUTH_TOKEN}` }
    })
        .then(parseJSONResponse)
        .then(data => {
            if (data.success) document.getElementById('project_code').value = data.project_code || data.code || '';
    ...
    fetch(`${window.GATEWAY_URL}workforce/employees?status=Active`, {
        headers: { 'Authorization': `Bearer ${window.AUTH_TOKEN}` }
    })
        .then(parseJSONResponse)
        .then(data => {
...
function handleEditProject(projectId) {
    fetch(`${window.GATEWAY_URL}project/projects/${projectId}`, {
        headers: { 'Authorization': `Bearer ${window.AUTH_TOKEN}` }
    })
        .then(parseJSONResponse)
        .then(data => {
...
function confirmDelete() {
    ...
    fetch(`${window.GATEWAY_URL}project/projects/${projectToDelete}`, { 
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${window.AUTH_TOKEN}` }
    })
        .then(parseJSONResponse)
...
document.getElementById('projectForm')?.addEventListener('submit', function(e) {
    ...
    const isUpdate = !!object.project_id;
    const method = isUpdate ? 'PUT' : 'POST';
    const url = isUpdate 
        ? `${window.GATEWAY_URL}project/projects/${object.project_id}` 
        : `${window.GATEWAY_URL}project/projects`;
